...
func _unhandled_input(_event: InputEvent) -> void:
	if Input.is_action_just_pressed("ui_accept"):
		DialogueManager.show_example_dialogue_balloon(load("res://dialogue/dog.dialogue"), "start")
		return
	if Input.is_action_just_pressed("narrator"):
		DialogueManager.show_example_dialogue_balloon(load("res://dialogue/Scene4.dialogue"), "start")
		return
